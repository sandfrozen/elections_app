class ElectionTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @election_type.save
        format.html { redirect_to election_types_path, notice: 'Typ został dodany' }
  ...
  def destroy
    if @election_type.elections.present? then
      respond_to do |format|
        format.html { redirect_to election_types_url, notice: 'Nie możesz usunąć typu wyborów ponieważ isteją powiązania.' }
        format.json { head :no_content }
      end
    else
      @election_type.destroy
      respond_to do |format|
        format.html { redirect_to election_types_url, notice: 'Typ wyborów został pomyślnie usunięty.' }
        format.json { head :no_content }
      end
    end
